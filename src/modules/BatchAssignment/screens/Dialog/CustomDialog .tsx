...
const CustomDialog = ({ onClose, solutionData }: Component) => {
    ...
    console.log(getStudents, "solution")
    return (
        <MOLFormDialog
        hideActionButton
            title=""
            ...
            size='large'
        >
            {solutionData?.assignment?.[0]?.studentId ?  <div className="grid grid-cols-12 border">
                <div className='col-span-2 mr-2 border'>
                    ...
                    </div>
                    {
                        getStudents?.map((el: any, ind: number) => {
                            return (
                                <>
                                    <div className="flex items-center gap-3 px-2 cursor-pointer" onClick={() => {
                                        setAssignmentSolution(el?.solutions)
                                        setSelectedId(el?.value)
                                    }}>
                                        <span>{ind + 1}.</span>
                                        <span className={`${el?.value === selectedId ? 'text-red-400 font-medium' : ''} capitalize`}>{el?.label}</span>
                                    </div></>
                            )
                        })
                    }
                ...
                <div className="col-span-10">
                    <div className='font-medium'>{solutionData?.questionLabel}</div>
                    <div>
                        <ViewSolutionScreen data={assignmentSolution} />
                    </div>
                </div>

            </div> : <div className='flex justify-center items-center h-[150px]  text-xl font-semibold text-red-800'>No Student has done it yet </div> }
